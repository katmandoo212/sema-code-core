...
| 特性 | C# | Java |
|---|---|---|
| WebSocket | `ClientWebSocket` | OkHttp `WebSocket` |
| JSON | Newtonsoft.Json | Jackson |
| 异步 | `Task` / `async-await` | `CompletableFuture` |
| 信号量 | `SemaphoreSlim` | `Semaphore` |
| 事件回调 | `Action<JToken?>` | `Consumer<JsonNode>` |
| 构建工具 | `dotnet` | Maven |
