...
| 命令执行 | ✓ | ✓ | Bash |
| 任务管理 | ✗（硬移除） | ✓ | TaskCreate, TaskUpdate 等 |
| 用户交互 | ✓ | ✓ | AskUserQuestion |
| 其他工具 | ✓| ✓ | Skill, Agent, ExitPlanMode |

### 限制机制说明

Plan 模式的限制分为两层：

- **硬限制**：Task 系列工具（TaskCreate、TaskUpdate 等）从工具列表中直接移除，AI 无法调用
...
**核心操作**：**按顺序启动 "Explore" 子代理。**

1.  **聚焦**：专注于理解用户请求和相关代码。
2.  **顺序启动**：
    *   每次只能启动一个 "Explore" 子代理，等待其完成后再启动下一个。
    *   **数量原则**：力求使用最少的子代理（通常1个即可）。最多不超过3个。
        *   **1个**：适用于任务范围明确（如已知文件路径）、进行小范围修改。
        *   **多个**：适用于范围不确定、涉及多个代码区域或需要了解现有模式的情况。
3.  **递进探索**：如果使用多个子代理，后一个子代理的搜索重点应基于前一个子代理的发现来调整。

#### 阶段二：设计 (Design)

**目标**：基于阶段一的探索结果，设计出实现方案。

**核心操作**：**启动 "Plan" 子代理来设计方案。**

*   **并行启动**：可以并行启动最多1个子代理。
*   **是否启动**：
    *   **默认启动**：对于大多数任务，都应启动至少1个"Plan"子代理，以验证理解和考虑替代方案。
    *   **可以跳过**：仅适用于真正微不足道的任务（如修正拼写错误、单行修改、简单重命名）。
*   **给子代理的提示**：在启动子代理时，需要提供全面的背景信息，包括：
    *   阶段一探索的发现（文件名、代码路径追踪）。
    *   用户的需求和约束条件。
    *   要求其产出一个详细的实施方案。
...
| 选项 | 行为 |
|------|------|
| `startEditing` | 切换到 Agent 模式，**保留**对话历史，Task 工具重新可用 |
